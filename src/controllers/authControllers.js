...
const register = async (request, response) => {
  try {
    const user = await authService.register(request.body.username, request.body.password);
...
const validateToken = async (request, response) => {
  try {
    ...
    return response.status(200).json({ data:decodedToken, status: 200, message: 'Token Verified' });
  } catch (error) {
    if (error instanceof HTTPError)
      return response.status(error.code).json({ status: error.code, message: error.message });
    return response.status(500).json({ status: 500, message: error.message });
